...
def test_dragon_mission_duration_days(self, frostbite) -> None:
    assert frostbite.mission_duration_days() == 14

def test_phoenix_mission_duration_days(self, ember) -> None:
    assert ember.mission_duration_days() == 7

def test_unicorn_mission_duration_days(self, stardust) -> None:
    assert stardust.mission_duration_days() == 3

def test_unicorn_send_on_mission_low_power(self, tinsel) -> None:
...
def test_unicorn_send_on_mission_high_power(self, stardust) -> None:
    tinsel.send_on_mission()
    assert stardust._in_Stable == False

def test_phoenix_resurrect
